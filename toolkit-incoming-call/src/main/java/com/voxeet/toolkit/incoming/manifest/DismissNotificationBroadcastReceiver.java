...
public class DismissNotificationBroadcastReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (null != invitationBundle && null != invitationBundle.conferenceId) {
            ...
            createPromise(invitationBundle.conferenceId).then(new PromiseExec<DeclineConferenceResultEvent, Object>() {
                @Override
    ...
    private Promise<DeclineConferenceResultEvent> createPromise(@NonNull String conferenceId) {
        ConferenceService conferenceService = VoxeetSdk.conference();
        ...
        if (null == sessionService || null == conferenceService) {
            return new Promise<>(new PromiseSolver<DeclineConferenceResultEvent>() {
                @Override
                public void onCall(@NonNull Solver<DeclineConferenceResultEvent> solver) {
                ...
                public void onCall(@NonNull final Solver<DeclineConferenceResultEvent> solver) {
                    sessionService.open().then(new PromiseExec<Boolean, Object>() {
                        ...
                        public void onCall(@Nullable Boolean result, @NonNull Solver internal_solver) {
                            conferenceService.decline(conferenceId).then(new PromiseExec<DeclineConferenceResultEvent, Object>() {
                                @Override
                                public void onCall(@Nullable DeclineConferenceResultEvent result, @NonNull Solver<Object> internal_solver) {
